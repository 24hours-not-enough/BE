...
public interface PlanRepository extends JpaRepository<Plan, Long>, PlanRepositoryCustom {
    ...
    @Query("select distinct p from Plan p inner join fetch p.members inner join fetch p.checkLists inner join fetch p.calendars inner join fetch p.user  where p.user.id =:userId order by p.createdAt") //where p.id=:planId
