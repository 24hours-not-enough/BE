...
public class PlanController {
    ...
    @PostMapping("/plan")
    public ResponseEntity<GetPlan> planAdd(@RequestBody PlanRequestDto.Regist registDto) {
        Long planId = planService.addPlan(registDto);
    ...
    public ResponseEntity<GetAllPlan> planList(@AuthenticationPrincipal UserDetailsImpl userDetails, @PageableDefault(size = 5) Pageable pageable) {
        List<PlanResponseDto> plan = planService.findPlan(pageable);
    ...
    public ResponseEntity<GetPlanOne> planOne(@PathVariable Long planId) {
        PlanResponseDto planOne = planService.findPlanOne(planId);
