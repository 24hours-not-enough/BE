...
public class CalendarController {
    ...
    public ResponseEntity<PlanResponseDto.ResponseNodata> CalendarUnLocked(@AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long planId){
        calendarService.addCalendarUnLock(planId, userDetails.getUser().getId());
        return new ResponseEntity<>(PlanResponseDto.ResponseNodata.builder()
                .msg("상세계획 내용 잠금 해지 완료!")
                .result("success")
                .build(),HttpStatus.OK);
    }
}
